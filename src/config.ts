...
function applyEnvironmentOverrides(config: Config): Config {
  ...
  if (githubOwner && githubRepo) {
    config.documentation.sources.public.repo = 
...
export function validateAuth(config: Config): void {
  ...
  for (const source of sources) {
    if (source.auth_required && !getAuthToken(source)) {
      const tokenName = source.priority > 1 ? 'INTERNAL_DOCS_TOKEN' : 'GITHUB_TOKEN';
      throw new Error(`Authentication required for source but ${tokenName} not provided`);
    }
  }
}